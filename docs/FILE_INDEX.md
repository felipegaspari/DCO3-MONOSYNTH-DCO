...
- `loop()` — Core 0: MIDI USB+DIN every iteration; Serial2 + USB CDC pumps on `timer1msFlag`; `preset_store_boot_task()` (one-shot ~1.5 s); LFO1/LFO2 ~50 µs; drift ~51 µs; `bench_poll_core0` + `mem_diag_poll_core0`. `__not_in_flash_func`.
  - **Called from:** Arduino framework (Core 0).
  - **When:** Forever.
- `loop1()` — Core 1: `microsTimer2`; noise fleet; auto/manual cal, **or** the `[CAL_VERIFY]` sweep when `calibrationVerifyRequested` (debug cmd 36), **or** ADSR + FIFO pop + `voice_task_main`; ends with `bench_service(1)` + `mem_diag_poll_core1`. The manual-cal branch runs the PIO work core 0 books for it — `calSyncNeutralRequested` → `setSyncMode()`, `pwCvProbeRequested` → `run_pw_cv_probe()` — since `pio_defer_service()` only runs on the play path. `__not_in_flash_func`.
  - **Called from:** Arduino framework (Core 1).
  - **When:** Forever.
...
- `setSyncMode()` — Rebuild sync topology via `assign_sm_mapping()` + `start_voice_sms()`; retrigger. Declared in `state_machines.h` (core 1 only).
  ...
  - **When:** Serial2 param; manual-cal entry, to unsync OSC1/OSC2 before the solo.
...
- `init_pio()` — `pio_sm_claim` pio0 SM0–2 + pio1 noise (and classic sub SM if no engine2); load free + one soft-sync poll image into pio0; classic sub programs into pio1 **or** `subosc2_init()` on pio2.
  - **Called from:** `setup1()`.
  - **When:** Boot Core1.
...
> lives in the shared library (`DCO-SHARED-LIBRARIES/`, reached as `_shared/`),
> and both sketches consume it. The sketch keeps five one-line shims:
> `autotune.h` → `_shared/autotune.h`, `autotune.ino` → `_shared/autotune_impl.h`,
> `autotune_search.ino` → `_shared/autotune_search_impl.h`, `FS.h` →
> `_shared/FS.h`, `FS.ino` → `_shared/FS_impl.h`. The file names below are the
> shared ones; `autotune_constants.h`, `autotune_context.h` and
> `autotune_measurement.h` are pulled in by `_shared/autotune.h` and have no shim.
> See [`_shared/README.md`](../_shared/README.md) for what the sketch must
> provide, and [`_shared/docs/CALIBRATION_STORAGE.md`](../_shared/docs/CALIBRATION_STORAGE.md)
> for the on-flash bank format.
...
- `disable_all_oscillators_and_range_pwm()` — Mute oscs / park RANGE (PIO `range_pio_set_level(DIV_COUNTER)` when `RANGE0_PIO_DITHER_TEST`, else GPIO high); calls `reset_pw_to_DIV_COUNTER_PW` and clears `g_lastDrivenFreqHz` (nothing is running any more).
  ...
  - **When:** Cal setup.
- `reset_pw_to_DIV_COUNTER_PW()` — Shared PW PWM → max wrap. A rail, not an operating point: only the PW searches (which program PW per probe) may leave it here.
...
- `run_pw_cv_probe()` — `[PW_PROBE]` sweep of every PW channel through 0 / ¼ / ½ / ¾ / full scale on the soloed oscillator, printing the duty each level produces plus a per-channel `span≈…pp` and a verdict: expected channel moves = CV live, another channel moves = `PW_PINS` mismapped, nothing moves = no CV path to that pulse. Only channel 0 is wired here, so the other two are reported as skipped. Leaves PW clobbered; the next manual-cal pass rewrites it. Cancelable.
...
see through the `.ino` shim). Sizes on this board: `FSBankSize` 528 B
(`NUM_OSCILLATORS` 3 × 176), `FSPWBankSize` 6 B (`NUM_PW_CHANNELS` 3),
`FSManualOffsetBankSize` 3 B, `FSAmpComp440BankSize` / `FSAmpCompDutyOffsetBankSize`
6 B. Included from `include_all.h` **before** `amp_comp.h`, which sizes its arrays
with `chanLevelVoiceDataSize` from here. Format:
...
them. `ensure_pw_fs_banks()` and its `fs_file_size_ok()` helper are inside
`#if PROJECT_INSTRUMENT == 4` and **are not compiled on this board** — on DCO4
they repair PW banks left at the old 8-slot size; here they would overwrite a
measured PW center with defaults.

**Functions**
- `write_fs_bank()` — Truncate/create a LittleFS file and write a full bank in one shot.
  - **Called from:** `seed_fake_calibration_tables()`; `preset_bulk_commit()` for cal targets.
  - **When:** Fake seed; host bulk restore.
- `init_FS()` — Mount LittleFS; create any missing bank; read the leading `FS*BankSize` bytes of all seven banks (never the file's real length) and unpack into `ampCompArray` + float/Q8 frequency arrays, `PW_CENTER` / `PW_LOW_LIMIT` / `PW_HIGH_LIMIT` (`NUM_PW_CHANNELS` loop), `manualCalibrationOffset`, `ampComp440`, `ampCompDutyOffset`. Body under `ENABLE_FS_CALIBRATION`; idempotent.
...
- `update_FS_voice()` — Persist one osc amp table (seek + write one 176 B slice).
  - **Called from:** `DCO_calibration()` per osc.
  - **When:** Auto-cal.
- `update_FS_PWCenter()` — Persist PW center for one **PW channel** (`cal_pw_channel(osc)`, 0..2 here); out-of-range index returns silently.
...
- `pack_pw_u16()` — `static`. Pack one `u16` little-endian into a PW bank buffer at `channel * 2`.
  - **Called from:** `seed_fake_calibration_tables()`; `ensure_pw_fs_banks()` (DCO4 only).
  - **When:** Fake seed.
- `generate_fake_calibration_data()` — Build one osc’s 22 `[freq_x100, RANGE PWM]` pairs (archived curve shape, real note schedule). Per-osc spread from a fixed 8-entry `kOscScale`; this board uses `[0..2]`.
  - **Called from:** `seed_fake_calibration_tables()`.
  - **When:** Fake seed.
- `seed_fake_calibration_tables(force)` — Write full fake amp-comp + PW banks (centers from `kPwCenterDefault` in `globals.h`, low 0, high `DIV_COUNTER_PW`) + `AmpComp440` = 1400 to LittleFS (`"w"` truncate, which also repairs a wrong-sized leftover file), then `init_FS()`. Precomputes when `force=true`. Silent (no Serial). `force=false` only if `voiceTables` is missing.
...
- `preset_store_send_directory_to_mb()` — Send all 256 slots as `'O'` frames (`[slot][name:16]`, blank = unused) over `Serial2Dma`. Waits `serial2_dma_tx_ready()` per entry so the ping-pong cannot drop frames.
...
### `serial2_dma.h` / `serial2_dma.ino`

RP2350 UART1 TX DMA (Serial2 GP20/21 @ 2.5 Mbaud to Input). One claimed channel, ping-pong 256 B, `mutex_t` for Core0+Core1. Do not `Serial2.write()` after `serial2_dma_init()`. Serial1 MIDI and USB CDC are unchanged.

**Functions**
- `serial2_dma_init()` — Claim a DMA channel, DREQ_UART1_TX into `uart1->dr`.
  - **Called from:** `init_serial()` after `Serial2.begin(2500000)`.
  - **When:** Boot Core0.
- `serial2_dma_poll()` — If the in-flight buffer finished, kick the other ping-pong buffer.
  - **Called from:** start of `serial_panel_task()`; also from `serial2_dma_tx_ready()` / `serial2_dma_write()`.
  - **When:** Realtime Core0 (~1 ms) and before each TX.
- `serial2_dma_tx_ready()` — True if the fill buffer has room for one stuffed frame.
  - **Called from:** every Serial2 TX site (ADSRs, filter, `'p'`/`'x'`, `'L'`, `'O'` directory).
  - **When:** Before `serial_frame_write(Serial2Dma, …)`.
- `serial2_dma_write()` — Copy into the fill buffer and kick DMA if idle. `Serial2Dma.write()` wraps this.
  - **Called from:** `serial_frame_write(Serial2Dma, …)`.
  - **When:** Each Input-bound frame.

### `Serial.h`

Prototypes; defines `SERIAL_INNER_MAX_PAYLOAD` **36** (for `'B'` bulk chunk) before
including `serial_frame.h`. Includes `serial2_dma.h`. Declares block-echo helpers used after preset load.

### `Serial.ino`

**Functions**
- `init_serial()` — Serial1 MIDI baud (RX 1 / TX 0 @ 31250, IRQ/`setPollingMode(false)`), Serial2 2.5M Input link against the Input's `Serial1` (RX 21 from Input TX GP0, TX 20 into Input RX GP1); `serial2_dma_init()`; builds the O(1) command LUT (incl. `'B'`/`'C'`/`'N'`).
  - **Called from:** `setup()`.
  - **When:** Boot Core0.
...
- `input_handle_preset_dir_request()` — `'N'` → `preset_store_send_directory_to_mb()` (256× `'O'` frames on `Serial2Dma`).
  - **Called from:** parser LUT.
...
- `serial_panel_task()` — `serial2_dma_poll()` then non-blocking parser pump (`serial_parser_drain`). Sets ingress `PARAM_SRC_INPUT`. `__not_in_flash_func`.
...
- `serialSendParam16()` — Slim `'p'` TX via `serial_frame_write` (id + i16 LE, 3 B) out `Serial2Dma`. Drops if `!serial2_dma_tx_ready()`.
...
- `serialSendParam32()` — Slim `'x'` TX via `serial_frame_write` (id + u32 LE, 5 B) out `Serial2Dma` TX 20 into Input `Serial1` RX GP1 (gap 154, cal 155; Input relays 154 to Screen). Drops if `!serial2_dma_tx_ready()`, unless `force` (the live calibration gap), which waits for the DMA instead.
...
- `apply_param_manual_calibration_flag()` — Manual cal mode (resets `manualCalibrationStep` to 0 on entry); may `serialSendParam32` offsets. Entry saves `syncMode` / `softSyncChunks`, forces both to 0 (logged as `[MANUAL_CAL] sync neutralised: …` when sync was armed) and raises `calSyncNeutralRequested`, because the cal solo stops the partner and a synced slave cannot reset itself without a running master; exit restores them before `pio_defer_request_cal_restore()`, whose `start_voice_sms()` rebuilds the real topology.
